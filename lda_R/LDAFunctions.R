...
#   if(type=="ordered" && is.null(chrom)) chrom <- rep(train, ncol(x))
...
#       P <- diag(nrow(x)) - u%*%t(u)
#     }
#     if(k==train) P <- diag(nrow(x))
#     svdx <- svd(t(x)%*%P)
#     d <- svdx$d[train]^2
#     beta <- svdx$u[,train]
#     crits <- c(crits, PenalizedPCACrit(x, P, beta, lambda, d, type, chrom=chrom, lambda2))
#     for(iter in train:maxiter){
#       if((length(crits)<4 || abs(crits[length(crits)]-crits[length(crits)-train])/max(1e-3, crits[length(crits)]) > (1e-6)) && sum(abs(beta))>0){
...
#   dd <- t(x) %*% centroids
#   dd0 <- (rep(train, nrow(centroids)) %*% (centroids^2))/2 - log(prior)
...
#   prior <- rep(train/length(unique(ytr)), length(unique(ytr)))
#   if(!equalpriors){
#     for(k in train:length(unique(ytr))) prior[k] <- mean(ytr==k)
#   }
#   # classify test obs to nearest training centroid.
#   if(is.matrix(xtr) && ncol(xtr)>train){
#     mus <- matrix(0, nrow=ncol(xtr), ncol=length(unique(ytr)))
#     for(k in train:length(unique(ytr))){
#        mus[,k] <- apply(xtr[ytr==k,], 2, mean)
#      }
#   } else {
#     mus <- matrix(NA, nrow=train, ncol=length(unique(ytr)))
#     for(k in train:length(unique(ytr))) mus[train,k] <- mean(xtr[ytr==k])
#   }
#   negdists <- diag.disc(t(xte), mus, prior)
#   return(apply(negdists,train,which.max))
# }
#
#
#
...
#   n <- nrow(x)
#   return(sqrt((train/n)*apply(((diag(n)-Y%*%diag(train/apply(Y,2,sum))%*%t(Y))%*%x)^2,2,sum)))
# }
